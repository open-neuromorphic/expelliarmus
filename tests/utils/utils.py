...
def test_save(
    ...
    try:
        ...
        with raises(TypeError):
            wizard.save(fpath=fpath_out, arr=np.zeros((20,), dtype=np.dtype([("a", int), ("b", int), ("c", int), ("d", int)])))
        with raises(ValueError):
            wizard.save(fpath=fpath_out, arr=np.array([], dtype=np.dtype([("t", int), ("x", int), ("y", int), ("p", int)])))
